...
+ (Gaussian) VAE: Make $q(z)$ closer to $p(z)=\mathcal{N}(\mu,\sigma)$
+ $\mu,\sigma$ are learned. $\qquad Z\sim \mathcal{N}(\mu,\sigma)$
...
\begin{align*}
  ...
  \text{GumbelSoftmax}(x) &= \text{Softmax}( (x + g) / \tau )                         &\in [0,1]^N      \\
  g = \text{Gumbel}(0,1)  &= - \log (- \log \text{Uniform}(0,1))                      &\in \mathbb{R}^N \\
...
Cube-Like Space AE (under submission)

* Problematic Behavior of Latplan's SAE

[[png:zsae/unstable]]

+ Single real world state has several propositional states
+ *Unstable propositional symbols*

** Harmful effects of unstable propositonal symbols
...
* First-Order Logic Symbols. @@html:<br>@@ (Predicates, aka Relations)
...
+ _*Lacks evaluation on the truly difficult, combinatorial tasks*_
  ...
  + *15-Puzzle is significantly more difficult* : $!16\approx 10^{13}$ states
  + *SotA solves hard 24-Puzzle instances* : $!25\approx 10^{25}$ states
+ We need *SotA domain-indenpendent heuristics* to tacke the problem
  + As our domains are *automatically generated by NN*
...
* Heuristics Require a */Descriptive Action Model/* :

# I.e., *symbolic description* of the effect/preconditions

Action AutoEncoder is a black-box function

 [[png:aae/aae3]]

*Still incompatible to SotA heuristics!*

[TODO] picture

# SotA heuristics: The ones that wins the *International Planning Competition*.

SotA relaxation heuristics: *Landmark Cut* (relaxation of h+ into graph cut, AAAI '09)

SotA abstraction heuristics: *Bisimulation Merge-And-Shrink* (AAAI '11)

* Initial Attempt (on Arxiv): Double Stage AMA

Idea:

+ Train AAE to obtain the labeled action set
+ Train the Random Forest on the transitions
+ Convert Random Forest to a logical formula

* Did not Work!

* What is lacking in the AAE: Disentanglement of states and action in */apply/*

+ *AAE's Apply function:* $z_{t+1} = \text{apply}(z_t, a) = z_t + \text{effect}(z_t, a)$

+ *PDDL's Apply function:* $z_{t+1} = (z_t \setminus \text{eff}^-(a)) \cap \text{eff}^+(a)$

+ *Effects in STRIPS are separated from the current state $z_t$*

+ Issue: *No guarantee* that, in the learned latent space, $(z_t, z_{t+1})$ can be compactly expressed by

#+begin_center
 $z_{t+1} = (z_t \setminus \text{eff}^-(a)) \cap \text{eff}^+(a)$
#+end_center

+ \rightarrow We learn *states and actions at once (end-to-end)*
...
*Acknoledgements*: Christian Muise (coauthor), Hilde, Murat (for redirection), Akash
