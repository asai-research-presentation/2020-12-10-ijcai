...
When you do not need deliberation?

+ Image classification (single step)
+ Machine Translation (Just follow the most likely next word predictions, ignoring others. Never look back.)
...
\begin{align*}
  \text{GumbelSoftmax}(x) &= \text{Softmax}( (x + g) / \tau ) \\
...
+ *I have not solved truly difficult, combinatorial problems*
+ *8-Puzzle is super easy*  : state space is $!9=362880$ states
+ *15-Puzzle is significantly difficult* : $!16\approx 10^{13}$ states
+ *SotA only recently solves hard 24-Puzzle instances* : $!25\approx 10^{25}$ states
...
* Descriptive Action Model

In order to use h+, we need a *descriptive action model* that has a *symbolic* effect/precondition
...
+ PDDL's Apply function: $z_{t+1} = (z_t \setminus \text{eff}^-(a)) \cap \text{eff}^+(a)$
  ...
  *Effects in classical planning are separated from the current state $z_t$*

+ Issue: *No guarantee* that the learned $(z_t, z_{t+1})$ can be expressed by
