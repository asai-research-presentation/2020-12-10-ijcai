...
* Planners can *solve an 8-puzzle optimally << 0.1sec.*
...
* Latplan (Asai,Fukunaga 2018): Solving an Imaged-Based 8-puzzle @@html:<br>@@ */without any Prior Explicit Knowledge/* @@html:<br>@@ (i.e. No Human Annotation)
...
+ *Issue1*: *Effects must be disentangled from $z_t$*.
  ...
  #+begin_alignright
  (\rightarrow see paper, _/STRIPS = cube-like graph from graph-theory./_)
  #+end_alignright
...
#+begin_larger
#+begin_center
+ *Solution*: _Restrict $A$ to STRIPS_  +  _train $Z$ and $A$ end-to-end_
...
+ */Back-to-Logit/* : disentangles the states and actions *while learning the propositional space*
+ *Variety of SotA heuristic search methods became available*
+ *First demonstration of SotA heuristics scaling up the search in a learned representation*
  + The central focus of the community in the past >20 years
+ Unlike RL, it requires
  + *No policy learning* from the environment
  + *Domain-independent*; Not specific to any environment
  + *Guarantees optimality*
+ *Powerful SotA heuristics* will start beating RL with *0 training*!
