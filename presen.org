...
\[
  ...
  g= - \log (- \log \text{Uniform}(0,1)),\quad \tau \rightarrow 0
\]
...
+ *15-Puzzle is significantly difficult* : $!16=10^{13}$ states
+ *SotA only recently solves hard 24-Puzzle instances* (Bu and Korf, IJCAI19): $!25=10^{25}$ states
...
+ Issue: *No guarantee* that the learned $(z_t, z_{t+1})$ can be expressed by $z_{t+1} = (z_t \setminus \text{eff}^-(a)) \cap \text{eff}^+(a)$
