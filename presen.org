...
+ Fast Downward miserably fails *in the input processing*
...
+ Issue: the latent space *does not guarantee* that $(z_t, z_{t+1})$ can be compactly described by STRIPS

+ \rightarrow We learn *states and actions at once (end-to-end)* so that *the latent space is STRIPS-like*.
