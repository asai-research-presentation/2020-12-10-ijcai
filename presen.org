...
#+begin_larger
*Classical Planning in Deep Discrete Latent Space:@@html:<br>@@ Neural Symbolic Architectures and Symbol Grounding*
#+end_larger
...
+ the RL fanatics says "yes",
  "/intelligence emerges from RL.../"

#+end_center
#+end_span5
#+begin_span7
+ Or do you *think*, adjust, and take appropriate behavior?
...
+ A behavior learned from past experiences
+ Works *only when the learned knowledge is useful*
#+end_span6
#+end_row-fluid
#+end_container-fluid
...
+ State Representation -- States are comprehensive
...
# When deliberation is unnecessary?
# 
# + Image classification (*single step*)
# + Atari Game (*mostly reflex*)
# + Tasks that *completeness does not matter*
#   + Machine Translation (*An O.K. job is enough*)
#   + Games (*winning against a human is enough*)
...
# appeared to be the robber's first try, due to his lack of a previous
...
+ "/... removing the confused criminal from the gene pool. No one else was hurt./"
...
+ → */human-like intelligence/ is terrible* on average
+ → _We wish to build a *rational, deliberative agent*_

  operating on the _raw input (e.g. visual)_
...
* Humans are costly                                                :noexport:

#+begin_container-fluid
#+begin_row-fluid
#+begin_span6
How NASA operated its satellites before deploying its planner:
...
** Deployed Applications 1. Space                                  :noexport:
...
** Deployed Applications 2. Logistics/transportation               :noexport:
...
** Deployed Applications 3. Manufacturing, Robotics & Motion Planning :noexport:
...
** Deployed Applications 4. Others                                 :noexport:
...
* I do not buy into the hype                                       :noexport:
...
** Task: Solving an Imaged-Based 8-puzzle @@html:<br>@@ */without any Prior Explicit Knowledge/* @@html:<br>@@ (i.e. No Human Annotation)
...
#+begin_center
Agent performed some action to the before-state

resulting in the after state
#+end_center
...
An AutoEncoder that *enforces a certain distribution* on $Z \subset \mathbb{R}^n$.

Minimize $|x-f(x)|$ + *Kullback-Leibler Divergence* (KL loss)

+ $D_{\text{KL}}(p||q)$: distribution distance between $p$ and $q$
+ *Gaussian VAE*: $Z\sim q=\mathcal{N}(\mu,\sigma)$, make $q\rightarrow p=\mathcal{N}(0,1)$
+ Encoder generates $\mu,\sigma$, then $z=\mu+\sigma\cdot\epsilon$ where $\epsilon\sim\mathcal{N}(0,1)$
+ $D_{\text{KL}}(p||q)=1-\sigma+\log\sigma-\mu^2$

[[spng:ae-vae-embedding]]

# #+begin_quote
# You have 10k images of apples. A *Gaussian VAE* learns $Z = Encode(X) \approx N(\mu,\sigma)$ for some $\mu,\sigma \in \mathbb{R}^n$.
# #+end_quote
...
* Our Solution: Gumbel-Softmax VAE (Jang, ICLR2017)

$Z \sim \textbf{Categorical}$:

　　　→ $z\;$ _converges to a 1-hot vector_ e.g.  $\langle 0,0,1,0 \rangle$ .
...
* AMA_1 (Oracular) → */AMA_2 (Learning-based)/*
...
* Latplan's SAE has a Problematic Behavior: @@html:<br>@@ A /single/ observation has /several/ binary states

[[png:zsae/unstable]]

#+begin_xlarge
#+begin_center
+ *Unstable propositional symbols*
#+end_center
#+end_xlarge

** Unstable propositonal symbols considerd harmful

[[png:zsae/disconnected]]

+ *Harmful effect 1* : "Essential duplicates" are left undetected
  + violate "*do not turn any stones more than twice*"
+ *Harmful effect 2* : Search graph become disconnected
  + *Goal node becomes unreachable in the discrete space*

** Core finding: Symbol Stability.

#+begin_quote
  　 *Latent representations used by the symbolic solvers (e.g. planners) must be /stable/ : Unique against the external noise.*
#+end_quote

Regularizing the categorical variables to have *more zeros*

[[png:zsae/zsae-overview]]
...
* Unsupervised Grounding of First-Order Logic Symbols.  (Predicates, aka Relations)

#+begin_quote
*Finding the logical i.e. yes-no relations between object vectors.*
#+end_quote

[[png:fosae-overview/fosae-overview1]]
...
[[png:fosae-overview/fosae-overview2]]
...
[[png:fosae-overview/fosae-overview3]]
...
[[png:fosae-overview/fosae-overview]]


** First-Order State Auto Encoder

Key components are:
#+begin_quote
*Discrete (Gumbel-Softmax) Attention to extract the arguments.*

*Each predicate outputs a boolean value (yes/no) for a query.*

*Compatible to PDDL solvers.*
#+end_quote

[[spng:fosae]]

** CLEVR-based Blocksworld experiments.
...
* Basic Appendix for ML people                                     :noexport:
...
*Double-Stage AMA (Arxiv) /negative results/*

Cube-Space AE (IJCAI20)

* So far, what did I achieve?

+ *Visual input \rightarrow /propositional/ symbols*
+ *Unlabeled transitions \rightarrow /action/ symbols*
+ *Object vectors \rightarrow /predicate/ symbols*
+ _*Lacks evaluation on the truly difficult, combinatorial tasks*_
  + *8-Puzzle is easy*  : state space is $!9< 400k$ states
  + *15-Puzzle* : $!16\approx 10^{13}$ states
  + *SotA solves hard 24-Puzzle instances* : $!25\approx 10^{25}$ states
+ Requires *SotA heuristics* to tackle the problem
  + *domain-independent* : because our domains are *generated by NN*
  + *Core research focus* of planning community in the last 20 years.
...
** AMA_2 AAE/AD cheated with a custom planner

[[png:dsama/planning6]]

** Heuristics need */Descriptive Action Models/* : PDDL
...
#+begin_quote
#+begin_center
*/Descriptive action model/ is the next major milestone*

that */unleash the full potential/ of off-the-shelf planners*

*upon unstructured inputs!!!*
#+end_center
#+end_quote
...
** Good Accuracy, Ridiculously Large PDDL
...
** Fast Downward miserably fails @@html:<br>@@ /during the input processing/
...
+ _Flattening = Compiling a Decision Tree into_
  ...
  _an exponential number of Decision Lists!_
#+end_center
...
*Cube-Space AE (IJCAI20) (we address this)*

* Source of the problem in the AAE: @@html:<br>@@ _Entanglement_ between _states and effects_

*AAE's state progression:* $z_{t+1} = \text{apply}(z_t, a)$

*STRIPS state progression:* $z_{t+1} = (z_t \setminus \text{eff}^-(a)) \cap \text{eff}^+(a)$

+ Effects in STRIPS are *disentangled* from the current state $z_t$.

  (\rightarrow In the paper, we showed that STRIPS = *cube-like graph* from graph-theory.)

+ *Issue*: *No compact STRIPS model $A$ may exist* in $Z$

  #+begin_alignright
  $Z$ is already learned by the SAE and dumped into a fixed form
  #+end_alignright

+ *Solution*: _Train $Z$ and $A$ at once_ (end-to-end)

 　 _while restricting $A$ to be STRIPS._ Then *$Z$ must adapt*
...
** Proposed: */Back-to-Logit/* technique

+ (1) *Go to the /logit/ space by a monotonic transformation*,
+ (2) *perform a numeric addition*,
+ (3) *then /come back!/* 🚗🔥🔥🔥🔥🔥🕙🏛🗲🗲

[[png:space-ae/3-5]]

+ BatchNorm was chosen as the monotonic transformation.

** Extract the effects of action $a$ from the dataset
...
#+begin_row-fluid
#+begin_span8
$(z_{t,f},z_{t+1,f})=(0,1)\Rightarrow f\in\text{eff}^+(a)$.

$(z_{t,f},z_{t+1,f})=(1,0)\Rightarrow f\in\text{eff}^-(a)$.
...
based on *Theorem*: BtL guarantees that /add and delete-effect never coexist./

#+begin_quote
Proof sketch: Both BatchNorm and Sigmoid are monotonic, and we perform arithmetic additions.
When $(z_{t,f},z_{t+1,f})=(0,1)$, thus the numeric effect is positive, therefore
$(1,1)$ may happen but $(0,0)$ and $(1,0)$ cannot happen.
#+end_quote

** Extract the precondition of action $a$
...
* Current Ongoing Work (Join if anyone wants to collaborate!)

*Discrete, Logical Word Embedding for NLP*

Unsupervised Motion Grounding from Fine Time Series

　

Future work:

/Symbols are all you need/ -- Beating Atari Game with Mixed-Discrete-Continuous Latplan

* Other work from IBM Research on Neural Symbolic hybrid systems   :noexport:

[[png:nesy/nesy]]

** Ne-Sy Concept Learner
