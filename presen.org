...
+ ... removing the confused
  criminal from the gene pool.
  No one else was hurt.
  → */human-like intelligence/ is BS* (esp. average human)
...
+ Gaussian VAE: Assuming $Z\sim q=\mathcal{N}(\mu,\sigma)$, make $q\rightarrow p=\mathcal{N}(0,1)$
...
* Solution: Gumbel-Softmax VAE (Jang, ICLR2017)
...
** Min = delete effect / set intersection, Max = add effect / set union
...
** Extract the effects of action $a$ directly from the dataset

$\{ (z_t, z_{t+1}) \mid \text{action}(z_t, z_{t+1})=a\}$:

#+begin_container-fluid
#+begin_row-fluid
#+begin_span6
If $(0,1)$ : add-effect. If $(1,0)$ : delete-effect.
...
based on Theorem: /add-effect and delete-effect never coexists./

#+begin_quote
Proof sketch: BatchNorm is monotonic, and we perform arithmetic additions.
When $(z_{t,f},z_{t+1,f})=(0,1)$, thus the effect is positive, therefore
$(1,1)$ may happen but $(0,0)$ and $(1,0)$ cannot happen.
#+end_quote

** The precondition of action $a$ is also extracted directly

$\{ (z_t, z_{t+1}) \mid \text{action}(z_t, z_{t+1})=a\}$:

Extracts a *single* decision rule (not decision list nor trees)

corresponding to the bit value shared by all data points of this action $a$:
