...
* AutoEncoder (AE): Unsupervised Learning for NN                   :noexport:
...
* State AE = Discrete Variational AutoEncoders (VAEs)              :noexport:
...
* Action Model Acquisition (AMA2) Overview                         :noexport:
...
* Action Auto Encoder (AAE)                                        :noexport:
...
* AMA_3 : make AMA_2 _/STRIPS-compatible/_

#+begin_quote
*/AMA_2's progression:/* $z_{t+1} = \text{apply}(z_t, a)$ = black box
...
* AMA_3 = End-to-End NN: Cube-Space AE
...
** AMA_3 = End-to-End NN: Cube-Space AE                                    :noexport:
...
We proposed */Back-to-Logit/* method (replaces $\text{apply}$):
+ *Theorem*: */Back-to-Logit guarantees STRIPS./*
+ *Easy* to train + *Easy* to extract Effects & Preconditions

** Min = delete effect / set intersection @@html:<br>@@ Max = add effect / set union :noexport:
...
** Proposed: */Back-to-Logit/* = continuous effects                :noexport:
...
** Results: Better training (ablation study)                       :noexport:
...
* NN To PDDL (details ommited)                                     :noexport:

Effect & Precondition Extraction
...
* To PDDL: Extracting action effects (details ommited)             :noexport:
...
* To PDDL: Extracting action preconditions                         :noexport:
...
* Example PDDL output from our system                              :noexport:
...
* Results : Higher Success Rates + Better Scalability
...
* Results : Strong Generalization                                  :noexport:
